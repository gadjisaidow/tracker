...
export default function OpenPer({ navigation, route }) {
  ...
  const Start = () => {
    if (user != null) {
        if(sta == null) {
            fetch('https:/trackcom.ru/api/apper.php', {
                method: 'POST',
                headers: {
            ...
            alert('Сначала завершите уже активную перевозку');
        }
    } else {
    ...
    <ScrollView style={styles.container}>
        <View style={{ paddingLeft: 100, paddingRight: 100, paddingTop: 70}}>
              ...
              <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                  ...
                  <View>
                    <TouchableOpacity onPress={() => Start()} style={{ paddingTop: 0.8*vw, marginTop: 80, paddingBottom: 0.8*vw, paddingRight: 3*vw, paddingLeft: 3*vw, borderRadius: 7, backgroundColor: '#0d84fb' }}>
                      <Text style={{color: '#fff', fontSize: 0.9*vw, textAlign: 'center'}}>Начать перевозку</Text>
                    </TouchableOpacity>
                  </View>
              </View>
